        ...
        if cmd == "sm" and len(args):
          try:
            ...
            if to in developer or to in admin or to in assistant or to in whitelist or to in registered:
              ...
              notif.append(to)
            else: room.message("sumimasen <f x11FF0000='1'>"+to+"<f x11000000='1'> anata ,no register.", True)
...
############################################################### UNLOCK/LOCK BOT ######################################################################
        ...
        if cmd == "lock"and self.getAccess(room, user) >= 4:
          ...
          if args in self.roomnames:
            if self.getAccess(room, user) > 0: 
              locks.append(args)
              room.message("tidak bisa chat <f x11FF0000='1'> %s   <f x11000000='1'> karena <f x11FF0000='1'> room [locked] " % args, True)
        ...
        if cmd == "unlock" and self.getAccess(room, user) >= 4:
          ...
          if args in self.roomnames:
            if args in locks:
              if self.getAccess(room, user) > 0:
                locks.remove(args)
                room.message("room chat <f x11FF0000='1'> %s   <f x11000000='1'> is <f x1133FF33='1'> [unlock] " % args, True)
...
########################################################### BOT HABILITADO / DESABILITADO ############################################################
        ...
        if cmd == "sleep" and self.getAccess(room, user) >= 5:
         if self.getAccess(room, user) > 0: 
          if lockdown: return
          room.message(" <f x11000000='1'> Restia Mau <f x11FF0000='1'> [Tidur]  ", True)
          self.getRoom("restiabot")
          lockdown = False 
        ...
        if cmd == "wakeup" and self.getAccess(room, user) >= 5:
         if self.getAccess(room, user) > 0: 
           ...
           room.message("<f x11000000='1'> Restia sudah <f x1133FF33='1'> [Bangun] ", True)
           self.getRoom("restiabot")
            ...
            if self.getAccess(room, user) > 0:
              if user.name in wordtodaytime:
                ...
                if time.time() < w:
                  ...
                  string = ""
                  if days > 0:
                    string += str(days) + " " + (days == 1 and " <f x11000000='1'>days" or " <f x11000000='1'>hari" ) + "<f x11FF0000='1'> "
                  if len(string) > 0 or hours > 0:
                    string += str(hours) + " " + (hours == 1 and " <f x11000000='1'>hours" or " <f x11000000='1'>jam" ) + "<f x11FF0000='1'> "
                  if len(string) > 0 or minutes > 0:
                    string += str(minutes) + " " + (minutes == 1 and " <f x11000000='1'>minutes" or " <f x11000000='1'>menit" ) + " <f x11000000='1'><f x11FF0000='1'> "
                  string += str(seconds) + " " + (seconds == 1 and " <f x11000000='1'>seconds" or " <f x11000000='1'>detik" )
              ...
              if self.getAccess(room, user) > 0:
                ...
                room.message("anda sedang <f x11FF0000='1'>"+sntonick(user.name)+" <f x11000000='1'> merasakan! : <f x11FF0000='1'> "+str(x), True)
        ...
        if cmd =="family" and self.getAccess(room, user) >= 2:
            if self.getAccess(room, user) > 0:
              ...
              if self.getAccess(room, user) > 0:
                ...
                room.message("Keluarga dari <f x11FF0000='1'>  ["+user.name+"]  <f x11000000='1'> :D: <br/> ayah dari : <f x11FF0000='1'>"+user.name+"  <f x11000000='1'> istrinya : <f x11FF0000='1'>"+day+" <f x11000000='1'> anak Anda : <f x11FF0000='1'> "+dia, True)
...
########################################################## BLACKLIST #################################################################################
        ...
        if cmd.lower() == "bl" and self.getAccess(room, user) >= 5:
              ...
              if len(args) > 0:
                if "!anon" in args:
                    room.message("Anons tidak bisa menjadi blacklisted.")
                    return
                else:
                      if args not in blacklist:
                        ...
                        rank = self.getAccess(room, user)
                        if rank == 5:
                          room.message("<br/><br/> <f x11FE9A2E='1'> [5 - Admin]  : <f x11FF0000='1'>  "+user.name+"  <br/> <f x11FF0000='1'>  "+args+"   <f x11000000='1'> User anda berada di <f x11FF0000='1'> [BLACKLIST]    <f x11000000='1'>Sekarang :D", True)
                        if rank == 6:
                          room.message("<br/><br/> <f x11FE2EF7='1'> [6 - Developer]  : <f x11FF0000='1'>  "+user.name+"  <br/> <f x11FF0000='1'>  "+args+"   <f x11000000='1'> User anda berada di <f x11FF0000='1'> [BLACKLIST]    <f x11000000='1'>Sekarang :D", True)
                      else:
                        rank = self.getAccess(room, user)
                        if rank == 5:
                          room.message("<br/><br/> <f x11FE9A2E='1'> [5 - Admin]   : <f x11FF0000='1'>  "+user.name+"  <br/>  <f x11000000='1'>User Name <f x11FF0000='1'>  "+args+"   <f x11000000='1'> sudah di <f x11FF0000='1'> [BLACKLIST] ", True)
                        if rank == 6:
                          room.message("<br/><br/> <f x11FE2EF7='1'> [6 - Developer]  : <f x11FF0000='1'>  "+user.name+"  <br/>  <f x11000000='1'>User Name <f x11FF0000='1'>  "+args+"   <f x11000000='1'> sudah di <f x11FF0000='1'> [BLACKLIST] ", True)
              else:
                rank = self.getAccess(room, user)
                if rank == 5:
                  room.message("<br/><br/> <f x11FE9A2E='1'> [5 - Admin] : <f x11FF0000='1'>  "+user.name+"  <br/> <f x11000000='1'>Aku butuh <f x11FF0000='1'>[NICK]", True)
                if rank == 6:
                  room.message("<br/><br/> <f x11FE2EF7='1'> [6 - Developer]  : <f x11FF0000='1'>  "+user.name+"  <br/> <f x11000000='1'>Aku butuh <f x11FF0000='1'>[NICK]", True)
                ...
                if "!anon" in args:
                    ...
                    return
                      ...
                      if args in blacklist:
                        ...
                        room.message("<br/><br/> <f x116666CC='1'> [Desenvolvedor]  : <f x11FF0000='1'>  "+user.name+"  <br/> <f x11FF0000='1'>  "+args+"   <f x11000000='1'> Anda telah dihapus dari <f x11FF0000='1'> [BLACKLIST] ", True)
                      else:
                        room.message("<br/><br/> <f x116666CC='1'> [Desenvolvedor]  : <f x11FF0000='1'>  "+user.name+"  <br/>  <f x11000000='1'>User name <f x11FF0000='1'>  "+args+"   <f x11000000='1'> tidak <f x11FF0000='1'> [BLACKLIST] ", True)
              else:
                  room.message("<br/><br/> <f x116666CC='1'> [Desenvolvedor]  : <f x11FF0000='1'>  "+user.name+"  <br/> <f x11000000='1'>Aku butuh <f x11FF0000='1'>[NICK]", True)
        ...
        if cmd == "ban" or cmd == "banir" and self.getAccess(room, user) >= 4:
          if self.getAccess(room, user) > 0:
            if self.getAccess(room, user) > 0:
              ...
              room.message(" User   <f x11FF0000='1'>  "+args.title()+"   <f x11000000='1'>  itu dilarang.", True)
            else:
              room.message("Maaf Restia tidak memiliki (mod) di sini :(")
          else:
            room.message(" *warning* Anda tidak diizinkan untuk melakukan akses ini. *warning* ")
            
        if cmd == "uban" or cmd == "unban" and self.getAccess(room, user) >= 4:
          if args == "": return
          args = args.lower()
          if self.getAccess(room, user) > 0:
            if self.getAccess(room, user) > 0:
              if ch.User(args) not in room._getBanlist():
                room.message(" user   <f x11FF0000='1'>  "+args.title()+"   <f x11000000='1'>  Hal ini tidak bisa di akses   ", True)
                return
              room.unban(ch.User(args))
              room.message(" anda sudah di unban   <f x11FF0000='1'>  "+args.title()+"   <f x11000000='1'>  , tidak menyalahgunakan aturan lagi, spam , sara ,link share.  ", True)
            else:
              room.message("Maaf Restia tidak memiliki (mod) di sini :(")
          else:
...
################################################## ROOMS #################################################################################  
        ...
        if cmd == "join" and self.getAccess(room, user) >= 3:
           ...
           if args in self.roomnames:
            self.getRoom("restiabot")    
            room.message("<f x11000000='1'> Restia sudah di room dan <f x1133FF33='1'>Sedang Online sekarang", True)
           else:
             self.joinRoom(args)
             self.getRoom("restiabot")  
             room.message("  aku masuk :  <f x11FF0000='1'>  "+args.title(),True)
        ...
        if cmd == "leave" and self.getAccess(room, user) >= 5:
            if args == "":
              ...
              self.leaveRoom(args)
              self.getRoom("restiabot")   
              room.message("Restia sudah keluar dari rooms :  <f x11FF0000='1'>  "+args.title(),True)
          ...
          for i in self.roomnames: 
            ...
            j.sort() 
          room.message("[ <f x11000000='1'> Restia sedang online di (<f x11FF0000='1'>%s<f x11000000='1'>)  <f x11000000='1'> room chat] : <f x11FF0000='1'>  "%(len(self.roomnames))+", ".join(j), True)
...
################################################## CMDS/RANK #############################################################################            
          ...
          if args == "":
            rank = self.getAccess(room, user)
            if rank == 1:
              room.message("<br/><br/><br/><br/>★CMDS★<br/>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'> <br/> rank:  <f x11000000='1'> [1 - Whitelist] <f x11FF0040='1'>"+"<br/> ★  [Lista de Comandos] ★ "+"<br/><f x11FA5882='1'> [check,suck,eat,kill,hug,kisses,waifu2,rb,rb2,sn,rn,df,mydf,wt,waifu,pdf,family,blist,rooms,say,rsay,rank,ranker,mynick,nick,seenick,gs,gis,yt,yt2,pm,level,profile,about,find,bgtime,cso,find,cs,dice,people,users,myip,isdown,pg,barry,mc,relaxar,fuder,sut]", True)
            if rank == 2:
              room.message("<br/><br/><br/><br/>★CMDS★<br/>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'> <br/> rank:  <f x1100A3EF='1'> [2 - Registered] <f x11FF0040='1'>"+"<br/> ★  [Lista de Comandos] ★ "+"<br/><f x11FA5882='1'>  [check,suck,eat,kill,hug,kisses,waifu2,rb,rb2,sn,rn,df,mydf,wt,waifu,pdf,family,blist,rooms,say,rsay,rank,ranker,mynick,nick,seenick,gs,gis,yt,yt2,pm,level,profile,about,find,bgtime,cso,find,cs,dice,people,users,myip,isdown,pg,barry,mc,relaxar,fuder,sut]", True)
            if rank == 3:
              room.message("<br/><br/><br/><br/>★CMDS★<br/>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'> <br/> rank:  <f x11A9D0F5='1'> [3 - Assistant] <f x11FF0040='1'>"+"<br/> ★  [Lista de Comandos] ★ "+"<br/><f x11FA5882='1'>  [check,suck,eat,kill,hug,kisses,waifu2,rb,rb2,sn,rn,df,mydf,wt,waifu,pdf,family,blist,join,rooms,say,rsay,rank,ranker,mynick,nick,seenick,gs,gis,yt,yt2,pm,level,profile,about,find,bgtime,cso,find,cs,dice,people,users,myip,isdown,pg,barry,mc,relaxar,fuder,sut]", True)
            if rank == 4:
              room.message("<br/><br/><br/><br/>★CMDS★<br/>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'> <br/> rank:  <f x119F81F7='1'> [4 - Mod] <f x11FF0040='1'>"+"<br/> ★  →[Lista de Comandos]←   ★ "+"<br/><f x11FA5882='1'>  [check,suck,eat,kill,hug,kisses,waifu2,rb,rb2,sn,rn,lock,unlock,df,mydf,wt,waifu,pdf,family,blist,ban,uban,join,rooms,say,rsay,rank,ranker,mynick,nick,seenick,gs,gis,yt,yt2,pm,modslevel,profile,about,find,bgtime,cso,find,cs,dice,people,users,myip,isdown,pg,barry,mc,relaxar,fuder,sut]", True)
            if rank == 5:
              room.message("<br/><br/><br/><br/>★CMDS★<br/>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'> <br/> rank:  <f x11FE9A2E='1'> [5 - Admin] <f x11FF0040='1'>"+"<br/> ★  →[Lista de Comandos]←   ★ "+"<br/><f x11FA5882='1'> [check,suck,eat,kill,hug,kisses,waifu2,rb,rb2,sn,rn,lock,unlock,sleep,wakeup,df,mydf,udf,wt,waifu,pdf,family,bl,ubl,blist,ban,uban,join,leave,rooms,restart,say,rsay,ranker,mynick,nick,ping,seenick,gs,gis,yt,yt2,pm,msg,fax,bc,pm,mods,bg,level,profile,about,find,bgtime,cso,find,cs,dice,people,users,myip,isdown,pg,barry,mc,relaxar,fuder,sut]", True)
            if rank == 6:
              room.message("<br/><br/><br/><br/>★CMDS★<br/>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'> <br/> rank:  <f x11FE2EF7='1'> [6 - Developer] <f x11FF0040='1'>"+"<br/> ★  →[Lista de Comandos]←   ★ "+"<br/><f x11FA5882='1'>[check,suck,eat,kill,hug,kisses,waifu2,rb,rb2,sn,rn,lock,unlock,sleep,wakeup,shutdown,df,mydf,udf,wt,waifu,pdf,family,bl,ubl,blist,ban,uban,join,leave,rooms,shutdown,restart,e,save,say,rsay,ping,rank,ranker,setrank,mynick,nick,seenick,gs,gis,yt,yt2,msg,fax,bc,pm,mods,bg,level,profile,about,find,bgtime,cso,find,cs,dice,people,users,myip,isdown,pg,barry,mc,relaxar,fuder,sut]", True)
        elif cmd == "rank":
          if args == "":
            rank = self.getAccess(room, user)
            if rank == 1:
              room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'><br/>rank: <f x11000000='1'> [1 - Whitelist] <f x11FF0040='1'>", True)
            if rank == 2:
              room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'><br/>rank: <f x1100A3EF='1'> [2 - Registered] <f x11FF0040='1'>", True)
            if rank == 3:
              room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'><br/>rank: <f x11A9D0F5='1'> [3 - Assistant] <f x11FF0040='1'>", True)
            if rank == 4:
              room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'><br/>rank: <f x119F81F7='1'> [4 - Mod] <f x11FF0040='1'>", True)
            if rank == 5:
              room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'><br/>rank: <f x11FE9A2E='1'> [5 - Admin] <f x11FF0040='1'>", True)
            if rank == 6:
              room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+user.name+" <f x11000000='1'><br/>rank: <f x11FE2EF7='1'> [6 - Developer] <f x11FF0040='1'>", True)
        
          else:
              rank = self.getAccess(room, ch.User(args))
              if rank == 0:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x1181BEF7='1'> [0 - Unranked] <f x11FF0040='1'>", True)
              if rank == 1:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x11000000='1'> [1 - Whitelist] <f x11FF0040='1'>", True)
              if rank == 2:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x1100A3EF='1'> [2 - Registered] <f x11FF0040='1'>", True)
              if rank == 3:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x11A9D0F5='1'> [3 - Assistant] <f x11FF0040='1'>", True)
              if rank == 4:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x119F81F7='1'> [4 - Mod] <f x11FF0040='1'>", True)
              if rank == 5:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x11FE9A2E='1'> [5 - Admin] <f x11FF0040='1'>", True)
              if rank == 6:
                room.message("<br/><br/>★ RANK ★<br/> <f x11000000='1'>User Name: <f x11FF0000='1'>"+args+" <f x11000000='1'><br/>rank: <f x11FE2EF7='1'> [6 - Developer] <f x11FF0040='1'>", True)

        if cmd == "ranker":
            room.message("<br/><br/><f x11FF0000='1'>            [ ★★★ RANK ALLY ★★★] "+"<br/><f x11FE2EF7='1'> [Developer]♤♤♤♤♤♤ :   <f x11A9D0F5='1'> %s " % (", ".join(developer))+"<br/><f x11FE9A2E='1'> [Admin]♤♤♤♤♤ :   <f x119F81F7='1'> %s " % (", ".join(admin))+"<br/><f x11FE9A2E='1'> [Mod]♤♤♤♤ :   <f x119F81F7='1'> %s " % (", ".join(mod))+"<br/><f x11A9D0F5='1'> [Assistant]♤♤♤ :   <f x11FE2EF7='1'> %s " % (", ".join(assistant))+"<br/><f x11A9D0F5='1'> [Registered]♤♤ :   <f x11FE2EF7='1'> %s " % (", ".join(registered)), True)
...
#           try:
            ...
            if rank == 1:
              ...
              if target in whitelist:
                room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke    <f x11000000='1'> [1 - Whitelist]    <f x11000000='1'>  Sekarang :D. ",True)
                return
            ...
            if rank == 2:
              if not user.name == "tanpaid": return #put your name there (if user.name == "yourname" and user.name == "anothername":) To prevent other Owners(in this case is Co-Owners) to set user's rank to Co-Owner.
              if target in registered:
                room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke <f x1100A3EF='1'> [Registered]    <f x11000000='1'>  Sekarang :D. ",True)
                return
              if target in blacklist:
                blacklist.pop(target)
              if target in admin:
              ...
              room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke <f x1100A3EF='1'> [2 - Registered]    <f x11000000='1'>  Sekarang :D. ", True)
            if rank == 3:
              if not user.name == "tanpaid": return #put your name there (if user.name == "yourname" and user.name == "anothername":) To prevent other Owners(in this case is Co-Owners) to set user's rank to Co-Owner.
              if target in assistant:
                room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke    <f x11A9D0F5='1'> [3 - Assistant]    <f x11000000='1'>  Sekarang :D. ",True)
                return
              if target in blacklist:
                blacklist.pop(target)
              if target in admin:
            ...
            if rank == 4:
              if not user.name == "tanpaid": return #put your name there (if user.name == "yourname" and user.name == "anothername":) To prevent other Owners(in this case is Co-Owners) to set user's rank to Co-Owner.
              if target in mod:
                room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke    <f x119F81F7='1'> [4 - Mod]    <f x11000000='1'>  Sekarang :D. ",True)
                return
              if target in blacklist:
                blacklist.pop(target)
              if target in admin:
              ...
              room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke    <f x119F81F7='1'> [4 - Mod]    <f x11000000='1'>  Sekarang :D. ", True)
            if rank == 5:
              if not user.name == "tanpaid" and user.name == "restiabot": return #put your name there (if user.name == "yourname" and user.name == "anothername":) To prevent other Owners(in this case is Co-Owners) to set user's rank to Co-Owner.
              if target in admin:
                room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke    <f x11FE9A2E='1'> [5 - Admin]   <f x11000000='1'>  Sekarang :D. ",True)
                return
              if target in blacklist:
                blacklist.pop(target)
              if target in admin:
            ...
            if rank == 6:
              if not user.name == "tanpaid": return #put your name there (if user.name == "yourname" and user.name == "anothername":) To prevent other Owners(in this case is Co-Owners) to set user's rank to Co-Owner.
              if target in developer:
                room.message("<f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> Selamat anda dipromosikan ke    <f x11FE2EF7='1'> [6 - Developer]   <f x11000000='1'>  Sekarang :D. ",True)
                return
              if target in blacklist:
                blacklist.pop(target)
              if target in admin:
            ...
            if rank == -1:
              ...
              room.message(" <f x11FF0000='1'>  "+target.title()+"   <f x11000000='1'> ,User anda berada di   <f x11FF0000='1'> [-1 - BLACKLIST]    <f x11000000='1'>  Sekarang :D. ", True)
#           except:
        ...
        ## if user.name in reg or user.name in friends or user.name in trusted or user.name in owners:
          ...
          try:
            if args in nicks:
              ...
              room.message("pemakai <f x11FF0000='1'>"+args+" <f x11000000='1'> tidak menemukan nick :|", True)
          except:
            return
...
################################################## CHATANGO SPECIAL ######################################################################
        ...
        if cmd == ("find") and len(args) > 0 and self.getAccess(room, user) >= 2:
          ...
          if not ch.User(name).roomnames:
            room.message("user tidak berada dalam room atau afk :(")
          else:
            room.message("User %s Saat ini dalam obrolan berikuts :  %s  " % (args, ", ".join(ch.User(name).roomnames)), True)
                ...
                def getBgtime(args):
                   ...
                   # Get the days, hours, etc:
                    ...
                    if len(string) > 0 or hours > 0:
                        string += str(hours) + " " + (hours == 1 and " <f x11000000='1'>hours" or " <f x11000000='1'>jam" ) + "<f x11FF0000='1'> "
                    if len(string) > 0 or minutes > 0:
                        string += str(minutes) + " " + (minutes == 1 and " <f x11000000='1'>minutes" or " <f x11000000='1'>menit" ) + "<f x11FF0000='1'> "
                    string += str(seconds) + " " + (seconds == 1 and " <f x11000000='1'>seconds" or " <f x11000000='1'>detik" )
                    if expired == True:
...
################################################## CMD NORMAIS ###########################################################################
        ...
        if cmd == "ping" and self.getAccess(room, user) >= 4:
         ...
         if args == "":
          ...
          if len(finale) <=40 :
            room.message("<f x11000000='1'>saat ini ada <f x11FF0000='1'>  %s   <f x11000000='1'>user yang sudah terdaftar di room chat : <f x11FF0000='1'>%s"% (len(finale),", ".join(finale)), True)
         if args != "":
           if args not in self.roomnames:
        ...
        if cmd == "rr":
            room.message(random.choice(["%s" % "<f x11FF0000='1'>  BANNG!!  <f x11FF0000='1'> "+user.name+" , <f x11000000='1'>  anda  <f x11FF0000='1'> meninggal!! ","<f x11FF0000='1'> *click*  <f x11FF0000='1'> "+user.name+" ,  <f x11000000='1'> anda  <f x1133FF33='1'> selamat!! "]), True)
        ...
        if cmd == "myip":
            room.message("<f x11FF0000='1'>  "+user.name+"    <f x11000000='1'>   tidak menumakan ip   <f x11FF0000='1'>  "+message.ip+" ", True)
          ...
          if len(args)>0:
            for line in urlreq.urlopen("http://www.downforeveryoneorjustme.com/"+args):
              ...
              if "It's just you." in decoder:
                room.message("Site <f x11FF0000='1'>"+args+"<f x11000000='1'> now <f x1133FF33='1'>[ONLINE]", True)
                break
              if "It's not just you!" in decoder:
                room.message("site <f x11FF0000='1'>"+args+"<f x11000000='1'> now <f x11FF0000='1'>[OFFLINE]", True)
                break
              if "doesn't look like a site on the interwho." in decoder:
                room.message("lik dari site: <f x11FF0000='1'>"+args+"<f x11000000='1'> sekarang <f x116633FF='1'>tidak valid", True)
                break
              else:
                 continue
          else:
...
## Aisatsu or Greetings
    ...
    if "afk" == message.body:
      room.message (random.choice(["See ya, jangan lama lama "+sntonick(user.name)+" nanti kangen loh :*",]),True)
    if "bye" == message.body:
